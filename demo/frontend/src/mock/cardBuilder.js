// 从 backend/src/cardBuilder.js 原样 port 到浏览器（纯函数 · 无 Node deps）
// 唯一差异：从相对路径改为 @shared alias，便于浏览器 bundle
...
function buildContextLine({ signal, creator }) {
  ...
  const tail = signal.raw_text ? `「${signal.raw_text}」` : `《${signal.video_title}》`;
...
export function buildCard({ match, intentResult, userId }) {
  ...
  const headlineTpl = action?.payload?.headline ?? `我 {time} 蹲的那件事 · 它来了`;
